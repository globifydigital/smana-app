...
class HyperPayService {
    ...
    async createCheckout(data) {
        try {
            ...
            const response = await this.client.post(path, params.toString());
            return response.data;
        }
        catch (error) {
            console.error('[HyperPay] Prepare checkout failed:', error.response?.data || error.message);
            throw new Error(error.response?.data?.result?.description ||
     ...
     */
    async getPaymentStatus(checkoutId, currency) {
        try {
     ...
     */
    async createRegistration(customerEmail, billingAddress) {
        try {
            const path = '/v1/checkouts';
     ...
     */
    async getRegistrationStatus(checkoutId) {
        try {
     ...
     */
    async payWithToken(registrationId, amount, currency) {
        try {
